...
pacman::p_load(
  shiny, shinydashboard, tidyverse, ggcorrplot, car, ggplot2, dplyr, plotly,
                 ...
                 box(
                   ...
                   fluidRow(
                     column(12, offset = 4,
                            ...
                            actionButton("reset_prediction", "Reset")
                     )
                   )
                 )
          )
                       ...
                       column(4,
                              conditionalPanel(
                                ...
                                box(
                                  ...
                                  plotOutput("feature_importance_mlr"),
                       ...
                       column(4,
                              conditionalPanel(
                                ...
                                box(
                                  ...
                                  solidHeader = TRUE,
                                  collapsible = TRUE,
                                  tableOutput("metric_rf"),
                                  plotOutput("residuals_rf"),
                                  plotOutput("fit_curve_rf"),
                                  plotOutput("feature_importance_rf"),
                       ...
                       column(4,
                              conditionalPanel(
                                ...
                                box(
                                  ...
                                  solidHeader = TRUE,
                                  collapsible = TRUE,
                                  tableOutput("metric_bt"),
                                  plotOutput("residuals_bt"),
                                  plotOutput("fit_curve_bt"),
                                  plotOutput("feature_importance_bt"),
                                  plotOutput("top_10_feature_plot_bt"),
                                  tableOutput("top_10_feature_table_bt")
                                )
                              )
                       )
                     )
...
server <- function(input, output) {
  ...
  observeEvent(input$start_prediction, {
    ...
    if ("Multiple Linear Regression" %in% algorithms) {
      ...
      output$top_10_feature_plot_mlr <- renderPlot({
        ...
        # Create a data frame for the top 10 coefficients
        ...
        ggplot(df, aes(x = reorder(Coefficient, -Magnitude), y = Magnitude)) +
          ...
          labs(title = "Top 10 Coefficients", x = "Magnitude", y = "Coefficient") +
      ...
      output$top_10_feature_table_mlr <- renderTable({
        ...
        coefficients_table <- summary(MLR)$coefficients
        # Extract top 10 coefficients
        top_10_coefficients <- coefficients_table[order(abs(coefficients_table[, "Estimate"]), decreasing = TRUE), ][1:10, ]
        top_10_coefficients
      })
    }
    ...
    if ("Random Forest" %in% algorithms) {
      ...
      output$top_10_feature_table_rf <- renderTable({
        ...
        # Use datatable to output the table
        datatable(top10, 
                  caption = "Top 10 Important Features (sorted by IncMSE)", 
                  options = list(dom = 't', 
                                 scrollX = TRUE,
                                 autoWidth = TRUE))
      })
    ...
    if ("Boosted Tree" %in% algorithms) {
      ...
      output$feature_importance_bt <- renderPlot({
        # Visualize variable importance
        summary(BT)
        plot(BT)
      })
      ...
      output$top_10_feature_plot_bt <- renderPlot({
        ...
        # Convert importance scores to a data frame
        imp_df <- data.frame(Feature = row.names(imp$var.names), 
      ...
      output$top_10_feature_table_bt <- renderTable({
        # Get feature importance
        imp <- summary(BT)
        ...
        # Use datatable to output the table
        datatable(top10, 
                  caption = "Top 10 Important Features (sorted by Relative Influence)", 
                  options = list(dom = 't', 
                                 scrollX = TRUE,
                                 autoWidth = TRUE))
      })
  ...
  # Respond to the click event of the "Reset" button
  observeEvent(input$reset_prediction, {
    # Reset all inputs to default values  
    updateCheckboxGroupInput(session, "predicting_algorithm",
                             selected = "Multiple Linear Regression")
    updateSliderInput(session, "split_ratio", value = 0.7)
    updateSelectInput(session, "step_direction", selected = "both") 
    updateSliderInput(session, "tree_depth", value = 5)
    updateSliderInput(session, "trees", value = 20)
    updateCheckboxGroupInput(session, "variables",
                             selected = c("town", "flat_type", "storey_range","flat_model", "floor_area_sqm", "remaining_lease"))
    updateSliderInput(session, "correlations", value = 0.5)
  })
